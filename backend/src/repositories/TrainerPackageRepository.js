var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { injectable } from "tsyringe";
import { TrainerPackage } from "../models/TrainerPackage.js";
import { BaseRepository } from "./BaseRepository.js";
let TrainerPackageRepository = class TrainerPackageRepository extends BaseRepository {
    constructor() {
        super(TrainerPackage);
    }
    findPackageByTrainer(trainerId) {
        return this.model.find({ trainerId: trainerId });
    }
    findActiveByTrainer(trainerId) {
        return this.model.find({ trainerId, isActive: true });
    }
    findPackageById(packageId) {
        return this.model.findById(packageId);
    }
    updatePackage(packageId, data) {
        return this.model.findByIdAndUpdate(packageId, data, { new: true });
    }
    deletePackage(packageId) {
        return this.model.findByIdAndUpdate(packageId, { isActive: false }).then(() => { });
    }
};
TrainerPackageRepository = __decorate([
    injectable(),
    __metadata("design:paramtypes", [])
], TrainerPackageRepository);
export { TrainerPackageRepository };
//# sourceMappingURL=TrainerPackageRepository.js.map