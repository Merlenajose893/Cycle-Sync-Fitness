...
const adminService = new AdminService(new ApiAdminRepository());
...
export const useAdminAuth = () => {
  ...
  const getPendingTrainers = async (): Promise<Trainer[]> => {
    try {
      ...
      if (axios.isAxiosError(error)) {
        setError(error.response?.data?.message || "Failed to fetch pending trainers");
      } else {
  ...
  const approveTrainer = async (trainerId: string) => {
    try {
      ...
      if (axios.isAxiosError(error)) {
        setError(error.response?.data?.message || "Failed to approve trainer");
      } else {
        setError("Unexpected error occurred");
      }
      throw error;
  ...
  const rejectTrainer = async (trainerId: string, reason: string) => {
    try {
      ...
      if (axios.isAxiosError(error)) {
        setError(error.response?.data?.message || "Failed to reject trainer");
      } else {
        setError("Unexpected error occurred");
      }
      throw error;
